(() => {
  ...
  const css = `
    ...
    .bottom-nav-card.is-current::before {
      opacity: 1;
    }
    ...
    .bottom-nav-card.is-current .bottom-nav-index {
    ...
    .bottom-nav-card.is-current .bottom-nav-game {
      color: rgba(255,255,255,.72);
    }
    ...
    @media (hover:hover) and (pointer:fine) {
      ...
      .bottom-nav-arrow.prev:hover {
        transform: translateX(-4px);
      }

      .bottom-nav-arrow.next:hover {
        transform: translateX(4px);
      }
    }
  ...
  function getCharacterName(character) {
    return character && (character.name || character.characterName || character.protagonist || 'Neznámá postava');
  }

  function getGameTitle(character) {
    return character && (character.gameTitle || character.game || character.title || '');
  }
